  ...
  if (
    test('exempts intentionally-unshipped skills from curated-skill coverage', () => {
      ...
      try {
        ...
        writeInstallProfilesManifest(testDir, {
          core: { description: 'Core', modules: ['skill-alpha'] },
          developer: { description: 'Developer', modules: ['skill-alpha'] },
          security: { description: 'Security', modules: ['skill-alpha'] },
          research: { description: 'Research', modules: ['skill-alpha'] },
          full: { description: 'Full', modules: ['skill-alpha'] }
        });
        writeSkillFixture(testDir, 'alpha', 'Alpha skill');
        ...
        for (const [constant, overridePath] of Object.entries(dirOverrides)) {
          const dirRegex = new RegExp(`const ${constant} = .*?;`);
          source = source.replace(dirRegex, `const ${constant} = ${JSON.stringify(overridePath)};`);
        }
